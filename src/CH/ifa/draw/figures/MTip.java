...
public class MTip extends AbstractLineDecoration{
    ...
    public MTip() {
        this(0.40, 10, 8);  // this(0.35, 15, 12);
    }

    /**
     * Constructs an arrow tip with the given angle and radius.
     */
    public MTip(double angle, double outerRadius, double innerRadius) {
        setAngle(angle);
    ...
    private Polygon outline(int x, int y, double direction) {
        ...
        shape.addPoint(x, y);
        addPointRelative(shape, x, y, getOuterRadius(), direction);
        addPointRelative(shape, x, y, getOuterRadius(), direction + 1.50);
        addPointRelative(shape, x, y, getOuterRadius(), direction - 1.50);
        addPointRelative(shape, x, y, getOuterRadius(), direction);

        addPointRelative(shape, x, y, 14, direction);
        addPointRelative(shape, x, y, 16, direction + .50);
        addPointRelative(shape, x, y, 16, direction - .50);
        addPointRelative(shape, x, y, 14, direction);
        shape.addPoint(x,y);
